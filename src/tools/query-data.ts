...
export function registerQueryData(server: McpServer, env?: QueryEnv) {
	...
	server.registerTool(
		"enrichr_query_data",
		{
			title: "Query Staged Enrichr Data",
			description:
				"Query staged Enrichr enrichment data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
			inputSchema: {
				data_access_id: z
					.string()
					.min(1)
				...
				sql: z
					...
					.min(1)
					.describe(
						"SQL query to execute against the staged data",
					),
				limit: z
					.number()
					.int()
